...
export default function PakarIndex() {
    ...
    // --- LOGIKA FILTERING & SEARCHING (FRONTEND) ---
    const filteredData = useMemo(() => {
        return data.filter((item) => {
            // Filter Jenis Pakar
            if (filterJenis && item.jenis_pakar !== filterJenis) return false;
            ...
            if (searchTerm) {
                ...
                return (
                    ...
                    (item.jurusan?.nama && item.jurusan.nama.toLowerCase().includes(term))
                );
            }
            return true;
        });
                    ...
                    <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-gray-200 p-6">
                        ...
                        <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-6 gap-4 border-b border-gray-100 pb-4">
                            <div className="flex flex-col">
                                <span className="text-gray-900 font-bold text-lg whitespace-nowrap">Daftar Pakar</span>
                                <span className="text-sm text-gray-500">Total: {filteredData.length} Pakar</span>
                            </div>
                            ...
                            <div className="flex flex-col sm:flex-row gap-3 w-full xl:w-auto items-start sm:items-center">
                                ...
                                <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                                    <TextInput
                                        type="text"
                                        placeholder="Cari nama, NIP, jurusan..."
                                        value={searchTerm}
                                    ...
                                    >
                                        <option value="">Semua Jenis Pakar</option>
                                ...
                                {/* TOMBOL TAMBAH */}
                                <div className="w-full sm:w-auto flex justify-end shrink-0 sm:ml-2">
                            ...
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead className="bg-gray-50">
                                    <tr>
                                        ...
                                        <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Nama & NIP</th>
                                        <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Jenis Pakar</th>
                                        <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Jurusan (Kaprodi)</th>
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {loading ? (
                                        ...
                                        <tr><td colSpan={5} className="p-8 text-center text-gray-500">Belum ada data pakar sesuai filter.</td></tr>
                                    ) : (
                                        filteredData.map((item, index) => (
                                            <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                                                ...
                                                <td className="px-6 py-4 align-top">
                                                    ...
                                                    <div className="text-xs font-mono text-gray-500 bg-gray-100 inline-block px-1 rounded mt-1">
                                                        {item.username}
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4 align-top">
                                                ...
                                                <td className="px-6 py-4 align-top text-sm">
                                                    {item.jenis_pakar === 'kaprodi' ? (
                                                        <span className="font-semibold text-gray-700">{item.jurusan?.nama || '-'}</span>
                                                    ) : (
                    ...
                    <div className="flex-1 overflow-y-auto p-6">
                        <div className="space-y-4">
                            ...
                            {form.jenis_pakar === 'kaprodi' && (
                                <div className="bg-emerald-50 p-4 rounded-lg border border-emerald-200 animate-fade-in-down">
                                    <InputLabel value="Pilih Jurusan yang Diampu" className="text-emerald-800 font-bold" required />
                                    <select
                                    ...
                                    <p className="text-xs text-emerald-600 mt-1">Kaprodi hanya dapat memvalidasi nilai untuk jurusan ini.</p>
                                </div>
                            )}
